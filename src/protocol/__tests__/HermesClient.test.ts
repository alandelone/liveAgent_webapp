...
describe("HermesClient (FEAT-002)", () => {
  ...
  it("includes lastSeq in CLIENT_HELLO when reconnecting after events were received", () => {
    ...
    expect(helloEvent.protocolVersion).toBe(2);
  ...
  it("accepts whitespace-formatted PONG control messages and keeps the connection alive", async () => {
    const eventBus = new HermesEventBus();
    const client = new HermesClient(
      {
        url: "ws://localhost:8765/ws",
    ...
    socket.simulateServerMessage('{"type": "PONG", "timestamp": 120}');
    await vi.advanceTimersByTimeAsync(50);

    expect(client.getState()).toBe("connected");
    expect(client.getMetrics().lastPingRttMs).not.toBeNull();
  });

  it("normalizes Blob frames in receive order before following JSON markers", async () => {
    const eventBus = new HermesEventBus();
    const observed: string[] = [];
    eventBus.onAudio((chunk) => observed.push(`audio:${chunk.byteLength}`));
    eventBus.on("TTS_END", () => observed.push("end"));
    const client = new HermesClient(
      {
        autoReconnect: false,
        WebSocketClass: FakeWebSocket as unknown as typeof WebSocket,
    ...
    const socket = (client as unknown as { ws: FakeWebSocket }).ws;
    socket.onmessage?.({ data: new Blob([new Uint8Array([1, 2, 3, 4])]) } as MessageEvent);
    ...
    expect(observed).toEqual(["audio:4", "end"]);
  });
});
